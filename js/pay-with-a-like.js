var wpmudev_pwal = jQuery.extend(wpmudev_pwal || {}, {
  ...
  load_browser_cookie: function() {
    if (wpmudev_pwal.cookies_loaded == true) return;

    wpmudev_pwal.cookies_loaded = true;

    if (pwal_data["cookie_key"] == undefined) return;
    ...
    if (pay_with_a_like_cookie != undefined && pay_with_a_like_cookie != "") {
      try {
        ...
        if (
          pay_with_a_like_cookie != undefined &&
          pay_with_a_like_cookie["data"] != undefined &&
          !jQuery.isEmptyObject(pay_with_a_like_cookie["data"])
        ) {
        ...
        if (pwal_data.debug == "true")
          console.log(
            "wpmudev_pwal.cookies: Error on JSON.parse: " + e.message
          );
      }
    } else {
      if (pwal_data.debug == "true") {
        console.log(
          "load_browser_cookie: cookie [" + pwal_data["cookie_key"] + "] EMPTY"
        );
      }
    }
  },
  ...
  check_buttons_to_likes: function() {
    ...
    if (
      Object.keys(wpmudev_pwal.buttons).length > 0 &&
      Object.keys(wpmudev_pwal.cookies).length > 0
    ) {
      var pwal_info_items = {};
      var pwal_urls = "";
      ...
      for (var pwal_id in wpmudev_pwal.buttons) {
        ...
        if (wpmudev_pwal.cookies[pwal_id] != undefined) {
          if (pwal_data.debug == "true") {
            console.log(
              "check_buttons_to_likes: previous like cookie found: [%o]",
              wpmudev_pwal.cookies[pwal_id]
            );
          }
  ...
  register_button_href: function(pwal_id, pwal_post_id, pwal_href) {
    ...
    if (wpmudev_pwal.buttons[pwal_id] == undefined) {
      ...
      if (wpmudev_pwal.cookies[pwal_id] != undefined) {
        ...
        var pwal_wrapper_el = jQuery(
          "#pwal_content_wrapper_" + pwal_info["id"]
        );
        ...
        pwal_data["options"]["sitewide"] != undefined &&
        pwal_data["options"]["sitewide"] == "true"
      ) {
        var pwal_wrapper_el = jQuery(
          "#pwal_content_wrapper_" + pwal_info["id"]
        );
    ...
    if (pwal_data.options["sitewide"] == "true") {
      for (var pwal_id in wpmudev_pwal.buttons) {
        ...
        if (pwal_info_items[pwal_id] == undefined) {
          ...
          if (pwal_data.debug == "true") {
            console.log(
              "handle_sitewide: adding button: [%o]",
              pwal_info_items
            );
          }
    ...
    if (Object.keys(pwal_info_items).length > 0) {
      jQuery.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
          console.log(
            "handle_button: error HTTP Status[" +
              jqXHR.status +
              "] " +
              errorThrown
          );
        ...
        success: function(reply_data) {
          if (reply_data != undefined) {
            ...
            if (
              reply_data["errorStatus"] != undefined &&
              reply_data["errorStatus"] == false
            ) {
              // We want to set the cookie since the server might not register is due to caching.
              if (
                reply_data["cookie"] != undefined &&
                !jQuery.isEmptyObject(reply_data["cookie"])
              ) {
                wpmudev_pwal.cookie(
                  pwal_data["cookie_key"],
                  JSON.stringify(reply_data["cookie"]),
                  {
                    path: pwal_data["COOKIEPATH"],
                    domain: pwal_data["COOKIEDOMAIN"]
                  }
                );
                ...
                if (
                  reply_data["cookie"]["data"] != undefined &&
                  !jQuery.isEmptyObject(reply_data["cookie"]["data"])
                ) {
              ...
              if (
                reply_data["pwal_info_items"] != undefined &&
                !jQuery.isEmptyObject(reply_data["pwal_info_items"])
              ) {
                var deferred_reload = false;

                for (var pwal_id in reply_data["pwal_info_items"]) {
                  if (!reply_data["pwal_info_items"].hasOwnProperty(pwal_id))
                    continue;
                  ...
                  if (
                    pwal_info["content_reload"] != undefined &&
                    pwal_info["content_reload"] == "ajax"
                  ) {
                    if (
                      pwal_info["content_id"] != undefined &&
                      pwal_info["content_id"] != "" &&
                      pwal_info["content"] != undefined
                    ) {
                      jQuery(
                        "#pwal_content_wrapper_" + pwal_info["content_id"]
                      ).replaceWith(pwal_info["content"]);
                    }
                  } else {
  ...
  google_plusone_init_js: function() {
    if (
      pwal_data.options["use_google"] == undefined ||
      pwal_data.options["use_google"] != "true"
    )
      return;

    if (pwal_data.debug == "true") {
      console.log(
        "google_plusone_init_js: use_google[" +
          pwal_data.options["use_google"] +
          "]"
      );
    }

    if (
      pwal_data["google-plusone-js"] != undefined &&
      pwal_data["google-plusone-js"] != ""
    ) {
      if (
        pwal_data["google_button_lang"] != undefined &&
        pwal_data["google_button_lang"] != ""
      ) {
        if (pwal_data.debug == "true") {
          console.log(
            "google: setting lang[%o]",
            pwal_data["google_button_lang"]
          );
        }
        window.___gcfg = {
  ...
  google_plus_callback_js: function(pwal_id, data) {
    ...
    if (data.state == "off") {
      if (pwal_data.debug == "true")
        console.log(
          "google: id[" + pwal_id + "] data.state[" + data.state + "]"
        );
      return false;
    } else {
      if (wpmudev_pwal.buttons[pwal_id] != undefined) {
  ...
  setup_linkedin_js: function(pwal_id) {
    if (
      pwal_data.options["use_linkedin"] == undefined ||
      pwal_data.options["use_linkedin"] != "true"
    )
      return;

    if (pwal_data.debug == "true") {
      console.log(
        "setup_linkedin_js: use_linkedin[" +
          pwal_data.options["use_linkedin"] +
          "]"
      );
    }
    ...
    if (typeof IN == "undefined") {
      if (
        pwal_data["linkedin-js"] != undefined &&
        pwal_data["linkedin-js"] != ""
      ) {
  ...
  setup_twitter_js: function() {
    if (
      pwal_data.options["use_twitter"] == undefined ||
      pwal_data.options["use_twitter"] != "true"
    )
      return;

    if (pwal_data.debug == "true") {
      console.log(
        "setup_twitter_js: use_twtter[" + pwal_data.options["use_twitter"] + "]"
      );
    }

    twttr.ready(function(twttr) {
      twttr.events.bind("tweet", function(event) {
        if (pwal_data.debug == "true") console.log("twitter: event[%o]", event);

        if (event.target.parentElement.id == undefined) return;
  ...
  setup_facebook_js: function() {
    if (
      pwal_data.options["use_facebook"] == undefined ||
      pwal_data.options["use_facebook"] != "true"
    ) {
      if (pwal_data.debug == "true") {
        console.log(
          "setup_facebook_js: use_facebook[" +
            pwal_data.options["use_facebook"] +
            "]"
        );
      }
      return;
    ...
    (function(d, s, id) {
      ...
      js.id = id;
      if (
        pwal_data["facebook-all-js"] != undefined &&
        pwal_data["facebook-all-js"] != ""
      )
        js.src = pwal_data["facebook-all-js"];
      else js.src = "//connect.facebook.net/en_US/all.js";
  ...
  facebook_after_load: function() {
    if (pwal_data.debug == "true")
      console.log(
        "facebook_after_load: load_facebook[" +
          pwal_data.options["load_facebook"] +
          "]"
      );

    if (
      pwal_data.options["load_facebook"] != undefined &&
      pwal_data.options["load_facebook"] == "true"
    ) {
      if (pwal_data.debug == "true")
      ...
      if (pwal_data.options["facebook_api_use"]) {
        if (pwal_data.debug == "true")
          console.log(
            "facebook_after_load: facebook API key[" +
              pwal_data.options["facebook_api_key"] +
              "]"
          );
    ...
    FB.Event.subscribe("edge.create", function(href) {
      if (href != undefined) {
        ...
        jQuery("div.pwal_container fb\\:like.pwal_facebook_iframe").each(
          function(index) {
            fb_like_div = this;
            ...
            if (wpmudev_pwal.buttons[pwal_id] != undefined) {
              ...
              if (fb_iframe_height > fb_span_height) {
                // Should be close to 179 but have seen this as low as 148 and as large as 211
                pwal_info = pwal_info_tmp;

                if (
                  pwal_data.options["show_facebook_comment_popup"] !=
                    undefined &&
                  pwal_data.options["show_facebook_comment_popup"] == "true"
                ) {
        ...
        if (pwal_info != undefined && pwal_info != "") {
          ...
          var pollTimerFB = window.setInterval(function() {
            ...
            jQuery("#pwal_container_" + pwal_info["content_id"] + " ul").css(
              "overflow",
              "visible"
            );
  ...
  facebook_auth_loop: function() {
    ...
    FB.getLoginStatus(function(response) {
      if (pwal_data.debug == "true") {
        console.log(
          "facebook_auth_loop: FB.getLoginStatus: response[%o]",
          response
        );
      }
      ...
      if (response.status === "connected") {
        FB.api("/me", function(response) {
          if (response != undefined && response != "") {
            if (pwal_data.debug == "true") {
              console.log(
                "facebook_auth_loop: FB.getLoginStatus: /me: response[%o]",
                response
              );
            }
        ...
        FB.login(
          function(response) {
            if (pwal_data.debug == "true") {
              console.log(
                "facebook_auth_loop: FB.getLoginStatus: not_authorized: response [%o]",
                response
              );
            }
            ...
            if (response.authResponse) {
              ...
              if (pwal_data.debug == "true") {
                console.log(
                  "facebook_auth_loop: User cancelled login or did not fully authorize."
                );
              }
            }
          },
        ...
        if (pwal_data.options["facebook_auth_polling"] == "yes") {
          var poll_interval =
            pwal_data.options["facebook_auth_polling_interval"];
          setTimeout(function() {
            wpmudev_pwal.facebook_auth_loop();
          }, poll_interval * 1000);
        }
  ...
  facebook_check_access: function() {
    ...
    if (Object.keys(pwal_data["options"]["facebook_fan_pages"]).length > 0) {
      ...
      if (pwal_data.debug == "true")
        console.log(
          "facebook_check_access: pwal_facebook_page_ids[" +
            pwal_data["options"]["facebook_fan_pages"] +
            "]"
        );

      var query_str =
        "SELECT page_id, uid FROM page_fan WHERE uid = me() AND page_id IN (" +
        pwal_data["options"]["facebook_fan_pages"] +
        ")";
      if (pwal_data.debug == "true")
        console.log(
          "facebook_check_access: page_fan query_str[" + query_str + "]"
        );
      ...
      FB.api(query, function(responses) {
        if (pwal_data.debug == "true")
          console.log(
            "facebook_check_access: FQL: responses page_fan: [%o]",
            responses
          );
        ...
        if (responses != undefined && responses != "" && responses.length > 0) {
          for (var i = 0; i < responses.length; i++) {
            ...
            if (
              ...
              if (pwal_data.debug == "true")
                console.log(
                  "facebook_check_access: response page_id match[" +
                    response["page_id"] +
                    "]"
                );
            }
  ...
  facebook_check_buttons: function() {
    if (Object.keys(wpmudev_pwal.buttons).length > 0) {
      ...
      for (var pwal_id in wpmudev_pwal.buttons) {
        ...
        if (pwal_info["liked"] != undefined && pwal_info["liked"] == true)
          continue;
      ...
      if (pwal_urls != "") {
        var fb_query =
          "SELECT url, user_id FROM url_like WHERE user_id = me() AND url IN (" +
          pwal_urls +
          ")";
        if (pwal_data.debug == "true") {
              ...
              for (var i = 0; i < responses.length; i++) {
                ...
                if (
                  ...
                  for (var pwal_id in wpmudev_pwal.buttons) {
                    ...
                    if (
                      response["url"] == pwal_info["href"] &&
                      response["user_id"] == wpmudev_pwal.fb_user["id"]
                    ) {
                      if (pwal_data.debug == "true") {
                        console.log(
                          "facebook_check_buttons: FQL: url_like URL [" +
                            pwal_info["href"] +
                            "] [%o] [%o]",
                          response,
                          wpmudev_pwal.fb_user
                        );
                      }
      ...
      } /* if (Object.keys(pwal_info_items).length > 0) */ else {
        wpmudev_pwal.handle_buttons(pwal_info_items);
      }
    }
  },
